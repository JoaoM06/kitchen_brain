  ...
  return (
    <SafeScreen>
      ...
      <ItemModal
        ...
        onUpdated={async () => { closeItemModal(); await loadData({ silent: true }); }}
        onDeleted={async () => { closeItemModal(); await loadData({ silent: true }); }}
      />
    </SafeScreen>
  );
}
...
function ItemModal({ visible, item, onClose, onUpdated, onDeleted }) {
  ...
  React.useEffect(() => {
    if (item) { setExpiry(item.expiry || ""); setLocation(null); }
  }, [item]);
  ...
  const handleSave = async () => {
    ...
    try {
      setSaving(true);
      await updateStockItem(item.id, { location: location || null, expiry_text: expiry || null });
      onUpdated && onUpdated();
    } catch (err) {
  ...
  const confirmDelete = () => {
    if (deleting) return;
    Alert.alert("Excluir item", `Tem certeza que deseja excluir "${item.name}" do estoque?`, [
    ...
    <Modal visible={visible} animationType="slide" transparent onRequestClose={onClose}>
      <View style={modalStyles.backdrop}>
        ...
        <View style={modalStyles.sheet}>
          ...
          <Text style={modalStyles.subtitle}>Local: {item.locationLabel || "—"}</Text>
          ...
          <View style={{ marginTop: 16 }}>
            ...
            <View style={modalStyles.rowChoices}>
              {["geladeira", "armário", "freezer"].map((loc) => (
                <Pressable
                  key={loc}
                  onPress={() => setLocation(loc)}
                  style={[modalStyles.chip, location === loc && modalStyles.chipActive]}
                >
                  <Text style={[modalStyles.chipText, location === loc && modalStyles.chipTextActive]}>{loc}</Text>
                </Pressable>
              ))}
            </View>
          </View>
          ...
          <View style={{ marginTop: 16 }}>
            ...
            <DefaultInput value={expiry} onChangeText={setExpiry} placeholder="Validade (ex: 05/11 ou 05/11/2025)" />
          </View>
          ...
          <View style={modalStyles.actionsRow}>
            <Pressable
              onPress={handleSave}
              disabled={saving}
              style={({ pressed }) => [modalStyles.saveButton, saving && { opacity: 0.7 }, pressed && !saving && { opacity: 0.85 }]}
            >
              {saving ? (
          ...
          <Pressable style={modalStyles.deleteRow} onPress={confirmDelete} disabled={deleting}>
...
const styles = StyleSheet.create({
  ...
  search: { backgroundColor: colors.inputBackground, borderRadius: 28, marginBottom: 14 },
  addBtn: { borderRadius: 28, paddingVertical: 14, alignSelf: "center", width: "86%", marginBottom: 18 },
  sectionTitle: { fontSize: 22, fontWeight: "800", color: colors.text, marginBottom: 10 },
  card: { backgroundColor: "#F8F8F8", borderRadius: 16, overflow: "hidden", borderWidth: 1, borderColor: "#EFEFEF" },
});

const modalStyles = StyleSheet.create({
  backdrop: { flex: 1, backgroundColor: "transparent", justifyContent: "flex-end" },
  sheet: { backgroundColor: "#fff", paddingHorizontal: 16, paddingTop: 12, paddingBottom: 24, borderTopLeftRadius: 20, borderTopRightRadius: 20 },
  header: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 4 },
  title: { fontSize: 18, fontWeight: "800", color: colors.text },
  subtitle: { fontSize: 13, color: colors.mutedText, marginTop: 2 },
  sectionLabel: { fontSize: 12, fontWeight: "700", color: colors.mutedText, marginBottom: 4 },
  ...
  saveButton: { borderRadius: 50, paddingVertical: 14, paddingHorizontal: 20, alignItems: "center", justifyContent: "center", backgroundColor: colors.primary },
  saveContent: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8 },
  saveText: { color: "#fff", fontWeight: "700", fontSize: 16 },
  deleteRow: { marginTop: 12, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8 },
