...
export default function RecipeHubScreen({ navigation }) {
  ...
  const stats = useMemo(() => {
    return {
      userPosts: communityPosts.length,
      liked: reactions.likedIds?.length || 0,
      saved: reactions.savedIds?.length || 0,
    };
  }, [communityPosts.length, reactions.likedIds, reactions.savedIds]);
  ...
  return (
    <SafeScreen>
      ...
      <View style={styles.statsRow}>
        <StatCard label="Suas postagens" value={stats.userPosts} icon="sparkles-outline" />
        <StatCard label="Curtidas dadas" value={stats.liked} icon="heart-outline" />
        <StatCard label="Receitas salvas" value={stats.saved} icon="bookmark-outline" />
      </View>
